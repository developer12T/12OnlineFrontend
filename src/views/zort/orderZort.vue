  ...
  >
    <div class="p-2">
      <div class="flex flex-col-reverse sm:flex-row justify-end items-center mb-2">
        ...
        <div
          class="flex items-center sm:order-1 mb-4 sm:mb-0 ml-4 bg-white rounded-lg p-2 shadow-md"
        >
      ...
      <div
        ...
        v-if="this.tabs === 'wait-tab' || this.tabs === 'payment-tab'"
      >
        <button
          @click="showPrintIframe()"
      ...
      <div
        class="flex items-center rounded-lg p-2"
        v-else-if="this.tabs === 'success-tab'"
      >
        <button
          @click="showPrintIframeSuccess()"
      ...
      >
        <div
          class="bg-white rounded-lg shadow-xl w-full max-w-[98vw] h-full max-h-[98vh] flex flex-col"
        >
          <div class="flex justify-between items-center p-1 pl-6 border-b">
            <span class="text-md font-medium text-gray-900"
              >พิมพ์เอกสาร {{ selected.length > 0 ? selected.length + " ใบ" : "" }}</span
            >
      ...
      <div v-if="tabs === 'success-tab'">
        <div
          class="flex justify-center mt-0 overflow-y-scroll shadow-md sm:rounded-lg"
          style="height: calc(100vh - 165px)"
        >
          <table
            class="w-full bg-white text-sm text-left rounded-lg text-gray-500 dark:text-gray-400"
          >
            <TableOrder
              :columns="tableSuccess"
              :data="filteredItems"
              :selected="selected"
              :isLoading="isLoading"
            ...
            >
              ...
              <template v-slot:status="{ row }">
                <div
                  ...
                  v-if="row.statusText !== 'พบข้อผิดพลาด'"
                >
                  <span
              ...
              <template v-slot:saleschannel="{ row }">
                <div class="flex items-center justify-center">
                  ...
                  />
                  <img
                    v-else-if="row.saleschannel === 'Amaze'"
                    src="/amaze.png"
                    width="25"
                    class="mr-1"
                  />
      ...
      <div v-else>
        <div
          class="flex justify-center mt-0 overflow-y-scroll shadow-md sm:rounded-lg custom-scrollbar"
          style="height: calc(100vh - 165px)"
        >
          <table
            class="w-full bg-white text-sm text-left rounded-lg text-gray-500 dark:text-gray-400"
...
export default {
  ...
  setup() {
    ...
    const showPrintIframe = () => {
      if (selected.value.length > 0) {
        iframeSrc.value = `http://58.181.206.156:8080/12Trading/zort_pdf/printReceiptOriginal.php?checklist=${selected.value}`;
        showIframe.value = true;
      }
    };

    const showPrintIframeAndCopy = () => {
      if (selected.value.length > 0) {
        // iframeSrc.value = `http://58.181.206.156:8080/12Trading/zort_pdf/printReceiptOriginalandCopy.php?checklist=${selected.value}`;

        // ✅ รวม ID ทั้งหมดเป็น string เดียว
        const idString = selected.value.join(",");

        // ✅ ตั้งชื่อ iframe target ให้ตรงกับ form
        ...
        form.method = "POST";
        form.action =
          "http://58.181.206.156:8080/12Trading/zort_pdf/printReceiptOriginalandCopy.php";
        form.target = iframeName;

        // ✅ เพิ่ม textarea สำหรับส่งข้อมูล
        const textarea = document.createElement("textarea");
        textarea.name = "checklist";
        textarea.value = idString;
        form.appendChild(textarea);

        // ✅ ส่ง form เข้าสู่ iframe
        document.body.appendChild(form);
        form.submit();
        document.body.removeChild(form);

        // ✅ แสดง iframe ในหน้า
        iframeSrc.value = ""; // ไม่ต้องใช้ src เพราะส่งผ่าน form แล้ว
